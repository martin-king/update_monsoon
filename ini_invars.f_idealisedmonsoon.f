...
C       Temperature at tropospheric levels
        DO K=3,KX
          ...
          ENDDO
        ENDDO
cnh: begin make the whole earth flat again
c       do j=1,il
c         do i=1,ix
c           phi0(i,j)=20.d0
c         enddo
c       enddo 
c       call truncg(ntrun,phi0,phis0)

c       call spec(phi0,phis)
c       if(ix.eq.iy*4) call trunct(phis)
cnh       call grid(phis,phis0,1)
cnh end
