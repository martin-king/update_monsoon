...
C--   2. Shallow convection
      ...
      DO J=1,NGP
        ...
        IF (DMSE.GE.0.0) THEN
          ...
          IF (DRH.GE.0.0) THEN
c mpk            FLUXQ          = FCNV*FSHCQ*QSAT(J,NLEV)*DRH
            FLUXQ          = FCNV*FSHCQ*QA(J,NLEV)*DRH
...
c mpk          FLUXQ          = FVDIQ2*QSAT(J,NL1)*DRH
          FLUXQ          = FVDIQ2*QA(J,NL1)*DRH
          QTENVD(J,NL1)  = FLUXQ*RSIG(NL1) 
          QTENVD(J,NLEV) =-FLUXQ*RSIG(NLEV)
      ...
      DO K=3,NLEV-2
        ...
        IF (SIGH(K).GT.0.5) THEN
          ...
          DO J=1,NGP
            ...
            IF (DRH.GE.DRH0) THEN
c mpk              FLUXQ        = FVDIQ2*QSAT(J,K)*DRH
               FLUXQ        = FVDIQ2*QA(J,K)*DRH
